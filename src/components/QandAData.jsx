export const cocQuestions = [
  {
    title: "行李要帶什麼?",
    content: (
      <span>
        大部分都買的到，帶那些有固定使用的牌子，一落地就會需要的e.g.
        保養品、洗面乳、乳液。
        <br />
        <strong>延長線一定要帶</strong>
        ，Ikea買的到的都不用帶，個人藥品、找藥局開旅遊藥(退燒、抗過敏、消炎止痛，綜合感冒藥)
        <br />
        不用帶棉被= =<br />
        Plus:旅行用沐浴乳、洗髮乳just in case
      </span>
    ),
  },
  {
    title: "現金應該帶多少?",
    content:
      "建議帶大約1000美金其實就可以，等你銀行開戶完成以後再請父母電匯、國際匯款就可以了",
  },
  {
    title: "要先辦台灣的信用卡/爸媽副卡嗎",
    content:
      "Jeremy:推，建議在出國前辦爸媽的副卡。美國大部分地方都可以刷卡，比帶現金更方便也更安全，記得確認信用卡是否收取國外手續費，出國前也要通知銀行，避免刷卡被凍結。",
  },
  {
    title: "COC需要托福／雅思成績嗎？",
    content:
      "需要，但並非強制要求只會影響到是否要多上ESL\nESL 是 English as Second Language 的簡稱，也就是專門為國際學生及非母語者所設計的英文課涵蓋聽力、口說練習、閱讀文章、構思建構Essay，要完成ESL或檢測分數達標才能開始上大學的英文課，ENGL101、ENGL103這兩門英文課是轉學的必修。",
    link: "https://www.canyons.edu/academics/isp/international_students/apply.php",
    linkText: "COC Application page",
  },
  {
    title: "我可以在校園工作嗎？",
    content:
      "可以，TLC、ISP常常在招人，尤其當你某個科目跟美國人比特別強、會說兩種語言(中文、英文)",
  },
  {
    title: "我如何轉學到 UC？",
    content:
      "諮詢AJ、Amy，維持GPA，多參加課外活動(e.g. FBLA、NASA HASP、志工)，每年10月開始申請11月底截止隔年4月中旬公布，建議8月開始寫PIQ",
  },
  {
    title: "轉學的 GPA 要求",
    content:
      "保持高 GPA（盡量全拿 A）是轉學成功關鍵，避免 C 以下成績以免拉低整體平均，每個科系的範圍不同，競爭的科系4.0只是入場券",
  },
  {
    title: "Santa Clarita安全嗎",
    content:
      "非常安全，Santa Clarita 是美國前十大適合居住的城市，物價低於灣區，且距離洛杉磯機場僅需要一小時開車時間，純白人環境，治安非常良好。",
  },
];

export const umcQuestions = [
  { title: "UMC vs Red Deer", content: "UMC較A\n Red Deer較B" },
];

export const redDeerQuestions = [
  {
    title: "RedDeer 申請大學管道？",
    content: (
      <ul>
        <li>
          請代辦公司（代辦人員會協助處理繳交申請文件 學生可以專心唸書考試）
        </li>
        <li>自己送出申請（要非常注意文件繳教日期，沒人幫忙審查文件</li>
      </ul>
    ),
  },
  {
    title: "RedDeer上課與考試方式？",
    content: (
      <ul>
        <li>大部分老師不會管上課秩序，需自律</li>
        <li>有些作業自由練習</li>
        <li>自己排申請大學需要和自己喜歡的課</li>
        <li>上課時間一堂90分鐘中午有一小時休息時間，一天最多四堂課</li>
        <li>沒有排課的時間可自由運用</li>
        <li>課堂小考或作業分數為課堂成績佔總成績70%</li>
        <li>學期末Diploma佔30%</li>
      </ul>
    ),
  },
  {
    title: "RedDeer 需要托福/雅思成績嗎？",
    content: "需要，取得6.0含或以上的成績，否則需要填寫免責聲明",
  },
];

export const usQuestions = [
  {
    title: "美國會很危險嗎",
    content:
      "保持警覺心沒有想像中那麼危險，多關注新聞避開市區(Downtown)的鬧區，不要隨便露出現金、放東西在桌上、跟奇怪的人Eye contact，you will be alright。",
  },
  {
    title: "美國會有很嚴重的種族歧視嗎",
    content:
      "假如在加州及較北邊的大州、城市，種族歧視的情況較少，南部及較小的城市會比較嚴重，但大部分人都不會特別去注意你是什麼種族。",
  },
  {
    title: "美國/加拿大怎麼挑",
    content:
      "這取決於你的科系、夢想，美國的治安環境都較差，生活費也較高，但以理工科而言出路更多樣、就在矽谷、科技中心附近，加拿大的環境較安全，生活費較低。",
  },
];

export const canadaQuestions = [
  { title: "Is Canda Expensive?", content: "depends" },
];
